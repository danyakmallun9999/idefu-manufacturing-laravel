        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 md:p-8 text-gray-900">
                    ...
                    @if ($errors->any())
                        <div class="bg-red-100 border border-red-500 text-red-700 px-4 py-3 rounded-lg mb-6" role="alert">
                    ...
                    <form method="POST" action="{{ route('orders.update', $order) }}" enctype="multipart/form-data"
                        x-data="{ productType: '{{ old('product_type', $order->product_type) }}' }">
                        @csrf
                        @method('PUT')
                                    ...
                                    <select name="customer_id" id="customer_id"
                                        ...
                                        @foreach ($customers as $customer)
                                            <option value="{{ $customer->id }}"
                                                @selected(old('customer_id', $order->customer_id) == $customer->id)>{{ $customer->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div>
                                <div>
                                    ...
                                    <div class="mt-2 space-x-4">
                                        <label class="inline-flex items-center">
                                            <input type="radio" name="product_type" value="tetap" x-model="productType"
                                                class="text-indigo-600 border-gray-300 focus:ring-indigo-500">
                                            <span class="ml-2">Produk Tetap</span>
                                        </label>
                                        <label class="inline-flex items-center">
                                            <input type="radio" name="product_type" value="custom" x-model="productType"
                                                class="text-indigo-600 border-gray-300 focus:ring-indigo-500">
                                    ...
                                    <select name="product_id" id="product_id"
                                        ...
                                        @foreach ($products as $product)
                                            <option value="{{ $product->id }}"
                                                @selected(old('product_id', $order->product_id) == $product->id)>{{ $product->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div x-show="productType === 'custom'" x-transition class="mt-4 space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <textarea id="custom_product_specification" name="custom_product_specification" rows="4"
                                            class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"
                                            placeholder="Detail spesifikasi produk custom">{{ old('custom_product_specification', $order->product_specification) }}</textarea>
                                    </div>
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-500 mt-1">Upload gambar baru jika ingin mengganti (Format: jpeg, png, jpg, gif, webp. Maksimal 5MB)</p>
                                        @if ($order->image)
                                            <p class="text-sm text-gray-600 mt-1">Gambar saat ini:
                                                <a href="{{ $order->image_url }}" target="_blank" class="text-indigo-600 hover:text-indigo-900 underline">Lihat gambar</a>
                                            </p>
                                        @endif
                                        @error('custom_image')
                                ...
                                <div x-show="productType === 'tetap'" x-transition class="mt-4">
                                    ...
                                    <textarea id="fixed_product_specification" name="fixed_product_specification" rows="4"
                                        class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"
                                        placeholder="Tambahkan spesifikasi khusus jika diperlukan">{{ old('fixed_product_specification', $order->product_specification) }}</textarea>
                                </div>
                            </div>
                        </div>
